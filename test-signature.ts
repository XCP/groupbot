...
async function testSignature() {
  ...
  console.log('Exact message:', JSON.stringify(result, null, 2));

  // Test with trailing space
  result = await verifyMessage(message + ' ', signature, address, { strict: false });
  console.log('\nWith trailing space:', JSON.stringify(result, null, 2));

  // Test with newline
  result = await verifyMessage(message + '\n', signature, address, { strict: false });
  console.log('\nWith newline:', JSON.stringify(result, null, 2));

  // Test with CRLF
  result = await verifyMessage(message + '\r\n', signature, address, { strict: false });
  console.log('\nWith CRLF:', JSON.stringify(result, null, 2));
}
